...
export default function Transactions() {
  ...
  async function load() {
    const { data, error } = await supabase.from("transactions").select("*").order("date", { ascending: false }).limit(20);
  ...
  async function create(e: React.FormEvent) {
    ...
    const insert = {
      ...
      split_percent_for_payer: form.split_method === "percent" ? (form.percent ? Number(form.percent) : null) : null,
      split_fixed_who: form.split_method === "fixed" ? form.fixedWho : null,
      split_fixed_amount: form.split_method === "fixed" ? (form.fixedAmount ? Number(form.fixedAmount) : null) : null,
    };
  ...
  return (
    <div className="space-y-4">
      ...
      <form onSubmit={create} className="bg-white rounded-xl p-4 shadow space-y-3">
        ...
        <div><label className="block text-sm">Kategori</label>
          <input className="w-full border rounded p-2" required
            value={form.category} onChange={e=>setForm({...form, category:e.target.value})}/>
        </div>
        <div><label className="block text-sm">Beskrivning</label>
        ...
        <div><label className="block text-sm">Betalare</label>
          <select className="w-full border rounded p-2"
            value={form.payer} onChange={e=>setForm({...form, payer:e.target.value as Member})}>
        ...
        <div className="flex items-center gap-2">
          <input id="chk_shared" type="checkbox" checked={form.is_shared}
            onChange={e=>setForm({...form, is_shared:e.target.checked})}/>
          <label htmlFor="chk_shared">Gemensam?</label>
        </div>

        <div><label className="block text-sm">Delningsmetod</label>
          <select className="w-full border rounded p-2"
            value={form.split_method} onChange={e=>setForm({...form, split_method:e.target.value as SplitMethod})}>
        ...
        {form.split_method === "percent" && (
          <div>
            <label className="block text-sm">Procent {form.payer === "lukas" ? "Lukas" : "Annie"} (0–1)</label>
            <input type="number" step="0.01" inputMode="decimal" className="w-full border rounded p-2"
        ...
        {form.split_method === "fixed" && (
          <div className="space-y-2">
            <div>
              ...
              <select className="w-full border rounded p-2"
                value={form.fixedWho} onChange={e=>setForm({...form, fixedWho:e.target.value as Member})}>
                <option value="lukas">Lukas</option>
                <option value="annie">Annie</option>
              </select>
            </div>
            <div>
              ...
              <input type="number" step="0.01" inputMode="decimal" className="w-full border rounded p-2"
                value={form.fixedAmount} onChange={e=>setForm({...form, fixedAmount:e.target.value})}/>
            </div>
          </div>
        )}
      ...
      <div className="bg-white rounded-xl p-4 shadow">
        ...
        <div className="space-y-2">
          {rows.length === 0 && <div className="text-gray-500">Inga utlägg ännu.</div>}
          {rows.map(t => (
            <div key={t.id} className="flex justify-between text-sm border-b pb-1">
              <div>
                <div className="font-medium">{t.category} – {t.amount} kr</div>
                <div className="text-gray-600">{t.date} · {t.description ?? ""}</div>
              </div>
